<template>
  <div class="header-layout">
    ...
    <span class="text-header-layout">Thêm mới thông tin nhân viên</span>
  </div>
    ...
    <form action="#" class="form-my-control">
      <table class="mar-t-10">
        <tr>
          ...
          <td>
            <label class="custom-file-upload mar-l-10">
      ...
      <div class="group-btn-submit">
        ...
        <button @click.prevent="createUser()" class="btn my-btn-success"><span>Tạo mới</span></button>
      </div>
    </form>
  </div>
</template>
...
const createUser = async () => {
  ...
  if (response.status == 200) {
    if (response.data.returnCode == 200) {
      Swal.fire('Thành Công', 'Tạo người dùng thành công!', 'success').then((result) => {
        if (result.isConfirmed) {
          routerPush("EmployeeManager")
        }
      })
    } else {
      Swal.fire('Thất bại', 'Tạo người dùng thất bại!', 'error');
    }
...
const myChanImg = async (event) => { 
  ...
  var oFReader = new FileReader();
    ...
    $('.custom-file-upload').css('margin-left', '15px');
}
</script>
<style scoped></style>